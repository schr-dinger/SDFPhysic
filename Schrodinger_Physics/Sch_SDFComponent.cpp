...
void USch_SDFComponent::BeginPlay()
{
    ...
    {
        if (auto* PhysicsManager = World->GetSubsystem<USch_SDFPhysicsSubsystem>())
        {
            if (UsePhysicManager)
            {
...
void USch_SDFComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    if (UWorld* World = GetWorld())
    {
        if (auto* PhysicsManager = World->GetSubsystem<USch_SDFPhysicsSubsystem>())
...
void USch_SDFComponent::OnUnregister()
{
    Super::OnUnregister();
}
...
void USch_SDFComponent::SyncAngularFromL()
{
    ...
    if (bLockRotX) L_body.X = 0.f;
    if (bLockRotY) L_body.Y = 0.f;
    if (bLockRotZ) L_body.Z = 0.f;
...
bool USch_SDFComponent::CheckCollision(USch_SDFComponent* Other, FVector& OutContactPoint, FVector& OutNormal, float& OutSignedDistance, int32 MaxIters)
{
    ...
    if (tEnter > tExit) return false;
        FVector Seed = FMath::Lerp(Cself, Cother, 0.5f * (tEnter + tExit));
...
bool USch_SDFComponent::CheckCollision_QueryOnly(USch_SDFComponent* Other, FVector& OutContactPoint_WS, FVector& OutNormal_WS, float& OutSignedDistance)
{
    OutContactPoint_WS = FVector::ZeroVector;
    OutNormal_WS = FVector::UpVector;
    OutSignedDistance = TNumericLimits<float>::Max();

    if (!Other || !GetOwner() || !Other->GetOwner())
        return false;

    const FBox AabbA = Sdf.LocalBounds.TransformBy(StaticMesh->GetComponentTransform());
    const FBox AabbB = Other->Sdf.LocalBounds.TransformBy(Other->StaticMesh->GetComponentTransform());
    if (!AabbA.Intersect(AabbB.ExpandBy(1.0f)))
    {
        // 멀리 떨어져 있으면 대개 false
        // return false;
    }

    //시드 겹치는 경우는 교집합 중심, 아니면 두 중심의 중점
    FVector seed = 0.5f * (AabbA.GetCenter() + AabbB.GetCenter());
    if (AabbA.Intersect(AabbB))
    {
        //FBox Overlap = AabbA.Overlap(AabbB) ? (AabbA.Overlap(AabbB).GetBox()) : FBox(); //
        FBox Overlap = AabbA.Overlap(AabbB);
        if (Overlap.IsValid) seed = Overlap.GetCenter();
    }

    FVector p = seed;

    auto QuerySdfA = [&](const FVector& P, float& d, FVector& n)->bool 
        {
            /*
            int32 totalSelf = Sdf.Dim.X * Sdf.Dim.Y * Sdf.Dim.Z;
            int32 idxA = IndexFromWorld(P);

            if (idxA < 0 || idxA >= totalSelf)
            {
                return false;
            }

            d = Sdf.Values[idxA];
            n = Sdf.Gradients[idxA];
            return true;
            */
            const int32 total = Sdf.Dim.X * Sdf.Dim.Y * Sdf.Dim.Z;
            const int32 idx = IndexFromWorld(P);
            if (idx < 0 || idx >= total || !Sdf.Values.IsValidIndex(idx)) return false;

            const FTransform Xf = StaticMesh->GetComponentTransform();
            const FMatrix IT = Xf.ToMatrixWithScale().InverseFast().GetTransposed();

            const FVector gL = Sdf.Gradients.IsValidIndex(idx) ? Sdf.Gradients[idx] : FVector::UpVector;
            const FVector gW = IT.TransformVector(gL);
            const float   k = gW.Size();
            if (k < KINDA_SMALL_NUMBER) return false;

            n = gW / k;                 // 월드 법선
            d = Sdf.Values[idx] / k;    // 거리 스케일 보정
            return true;
        };
    auto QuerySdfB = [&](const FVector& P, float& d, FVector& n)->bool 
        {
            /*
            int32 totalOther = Other->Sdf.Dim.X * Other->Sdf.Dim.Y * Other->Sdf.Dim.Z;
            int32 idxB = Other->IndexFromWorld(P);

            if (idxB < 0 || idxB >= totalOther)
            {
                return false;
            }

            d = Other->Sdf.Values[idxB];
            n = Other->Sdf.Gradients[idxB];
            return true;
            */
            const int32 total = Other->Sdf.Dim.X * Other->Sdf.Dim.Y * Other->Sdf.Dim.Z;
            const int32 idx = Other->IndexFromWorld(P);
            if (idx < 0 || idx >= total || !Other->Sdf.Values.IsValidIndex(idx)) return false;

            const FTransform Xf = Other->StaticMesh->GetComponentTransform();
            const FMatrix IT = Xf.ToMatrixWithScale().InverseFast().GetTransposed();

            const FVector gL = Other->Sdf.Gradients.IsValidIndex(idx) ? Other->Sdf.Gradients[idx] : FVector::UpVector;
            const FVector gW = IT.TransformVector(gL);
            const float   k = gW.Size();
            if (k < KINDA_SMALL_NUMBER) return false;

            n = gW / k;
            d = Other->Sdf.Values[idx] / k;
            return true;
        };

    float dA = 0.f, dB = 0.f;
    FVector nA = FVector::UpVector, nB = FVector::UpVector;

    //약간 체크
    const int32 kRefineIters = 2;
    for (int32 it = 0; it < kRefineIters; ++it)
    {
        if (!QuerySdfA(p, dA, nA)) break;
        if (!QuerySdfB(p, dB, nB)) break;

        // 어느 쪽 표면이 더 가까운지 보고 표면 쪽으로 한 스텝
        if (FMath::Abs(dA) <= FMath::Abs(dB))
        {
            //표면으로 투영
            if (!nA.Normalize()) break;
            p = p - nA * dA; // p를 A의 표면 쪽으로 이동
        }
        else
        {
            if (!nB.Normalize()) break;
            //표면의 "밖 방향"은 nB 이고, p를 B 표면으로 옮기려면 -nB*dB
            p = p - nB * dB;
        }
    }

    // 4) 최종 샘플
    if (!QuerySdfA(p, dA, nA)) return false;
    if (!QuerySdfB(p, dB, nB)) return false;
    const bool okA = nA.Normalize();
    const bool okB = nB.Normalize();
    if (!okA || !okB) return false;

    // 5) 선택 규칙:
    //    - 두 표면 중 "더 가까운" 쪽의 표면 포인트/법선을 접촉 정보로 사용
    //    - 법선은 "A에서 B를 향하는 방향"이 되도록 일관화
    if (FMath::Abs(dA) <= FMath::Abs(dB))
    {
        // A 표면이 더 가깝다 → CP는 A 표면, 법선은 A의 바깥쪽(nA)
        OutSignedDistance = dA;
        OutNormal_WS = nA;               // A의 바깥쪽
        OutContactPoint_WS = p - nA * dA;     // A 표면점으로 정밀 보정

        // 법선이 A->B로 향하도록 보정
        //const FVector AB = Other->GetOwner()->GetActorLocation() - GetOwner()->GetActorLocation();
        //if (FVector::DotProduct(OutNormal_WS, AB) < 0.f) OutNormal_WS = -OutNormal_WS;
    }
    else
    {
        // B 표면이 더 가깝다 → CP는 B 표면, 법선은 "A→B"가 되도록 -nB 사용
        OutSignedDistance = dB;
        OutNormal_WS = -nB;              // A에서 B를 향하게 (B의 바깥 nB의 반대)
        OutContactPoint_WS = p + nB * dB;     // B 표면점 (p - (-nB)*dB 와 동일)
    }
    ...
    // 6) 결과 해석
    //  OutSignedDistance < 0  → 침투(겹침)
    //  OutSignedDistance ≈ 0  → 접촉
    //  OutSignedDistance > 0  → 분리

    

    if (OutSignedDistance < 0)
    {
        if (GEngine)
        {
            GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Green, TEXT("Hit!"));
...
void USch_SDFComponent::VelocityIntegrate(float dt)
{
    if (!CanMove) return;

    FVector a_world = ApplyGravity();
    Velocity += a_world * dt;

    if (!AccumTorqueWorld.IsNearlyZero())
    {
        FQuat R = GetOwner()->GetActorQuat();
        FVector tau_body = R.UnrotateVector(AccumTorqueWorld);
        L_body += tau_body * dt;
...
void USch_SDFComponent::TransformIntergrate(float dt)
{
    if (!CanMove) return;
    ...
    if (GEngine)
    {
        //UE_LOG(LogTemp, Warning, TEXT("Debug Check V.size = % .6f v = (% .3f, % .3f, % .3f)"), Velocity.Size(), Velocity.X, Velocity.Y, Velocity.Z);
    }


    FVector x0 = Owner->GetActorLocation();
    FVector x1 = x0 + Velocity * dt;

    //UE_LOG(LogTemp, Log, TEXT("[%s] Moving: x0=%s, Velocity=%s, x1=%s"),
    //    *GetOwner()->GetName(), *x0.ToString(), *Velocity.ToString(), *x1.ToString());

    FQuat q0 = Owner->GetActorQuat();
    float w = AngularVelocity.Size();
    FQuat dq = FQuat::Identity;
    if (w > KINDA_SMALL_NUMBER)
    {
        const FVector axis = AngularVelocity / w;
        dq = FQuat(axis, w * dt);
    }
    const FQuat q1 = (dq * q0).GetNormalized();

    Owner->SetActorLocationAndRotation(x1, q1, false, nullptr, ETeleportType::None);

    SyncLFromAngular(); // ω_world -> L_body(로컬) 갱신
}
